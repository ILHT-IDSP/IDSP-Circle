...
export default function FriendSection({ formData, setFormData, friends, setFriends }: { formData; setFormData; friends; setFriends }) {
	...
	return (
		...
		>
			<h2 className='text-xl font-semibold mb-2 text-[var(--foreground)]'>added friends</h2>
			<div
			...
			>
				{friends.map(f => (
					<div
						key={f.id}
						className='flex items-center bg-transparent border border-[var(--border)] rounded-full px-3 py-1 text-[var(--foreground)] whitespace-nowrap'
					>
