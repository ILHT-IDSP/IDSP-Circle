	...
	return (
		<div className='fixed inset-0 bg-black bg-opacity-70 z-50 flex items-center justify-center p-4'>
			<div className='bg-white dark:bg-gray-800 rounded-xl w-full max-w-md overflow-hidden shadow-2xl'>
				...
				<div className='p-6'>
					<h2 className='text-2xl font-bold text-gray-900 dark:text-white mb-2'>{tutorialSteps[currentStep].title}</h2>
					<p className='text-gray-600 dark:text-gray-300 mb-8'>{tutorialSteps[currentStep].description}</p>
					...
					<div className='flex justify-between items-center'>
						...
						<div className='flex space-x-2'>
							{tutorialSteps.map((_, index) => (
								<div
									key={index}
									className={`h-2 w-2 rounded-full ${index === currentStep ? 'bg-circles-dark-blue' : 'bg-gray-300 dark:bg-gray-600'}`}
								/>
							))}
						</div>
